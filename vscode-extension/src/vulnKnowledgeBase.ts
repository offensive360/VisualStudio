...
export interface VulnKBEntry {
  ...
  howToFix: string;
  codePatternBad: string;
  codePatternGood: string;
  cwes: string[];
}

const KB_DATA: Record<string, VulnKBEntry> = {
  'sql_injection': {
    title: 'SQL Injection',
 ...
 */
export function lookupVuln(vulnType: string): VulnKBEntry | undefined {
  if (!vulnType) { return undefined; }
  ...
  // Direct match
  if (KB_DATA[key]) { return KB_DATA[key]; }
 ...
 */
export function getFixHint(vulnType: string): string {
 ...
 */
export function getFullHelp(vulnType: string): string {
  const entry = lookupVuln(vulnType);
  if (!entry) {
 ...
 */
export function filterReferences(references: string): string[] {
 ...
 */
export function getAuthErrorMessage(statusCode: number | null, isNetworkError: boolean): string {
  if (isNetworkError) {
